...
type openEditorMsg struct{ pr PullRequest }
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case openEditorMsg:
		...
		return m, tea.ExecProcess(c, func(err error) tea.Msg {
			...
			return actionDoneMsg{fmt.Sprintf("Opened editor for %s#%d", msg.pr.Repo, msg.pr.Number), false}
		})
...
func (m model) viewReview() string {
	...
	// Actions
	b.WriteString("  " + helpStyle.Render("a approve • c comment • x request changes • k checkout • e editor • s skip • o open in browser • q quit") + "\n")
